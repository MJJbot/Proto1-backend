...
var getUserResponseWithUID = async function(uid) {
   var user = await findUserWithUID(uid);

   var response = {
      userName: user.userName,
      userImg: user.userImg,
      channel: "https://www.twitch.tv/" + user.userLogin
   };
...
var findRecommendQAsWithUID = async function(uid) {
   var recommendQAs = await model.RecommendQA.find({uid: uid});
   if (recommendQAs.length > 0) {
      console.log('recommendQA: ', recommendQAs);
   } else {
      console.log('recommendQA with ', uid, ' is not in DB');
   }
   return recommendQAs;
};

var findRecommendQAWithUIDAndID = async function(uid, id) {
   var recommendQA = await model.RecommendQA.findOne({uid: uid, id: id});
   if (recommendQA) {
      console.log('recommendQA: ', recommendQA);
   } else {
      console.log('recommendQA with ', uid, ' is not in DB');
   }
   return recommendQA;
...
var deleteRecommendQAWithUIDAndID = async function(uid, id) {
   var result = await model.RecommendQA.deleteOne({uid: uid, id: id});
...
var getRecommendQAResponseWithUID = async function(uid) {
   var user = await findUserWithUID(uid);
   var recommendQAs = await findRecommendQAsWithUID(uid);
...
module.exports = {
   ...
   getUserResponseWithUID: getUserResponseWithUID,
   ...
   findRecommendQAsWithUID: findRecommendQAsWithUID,
   findRecommendQAWithUIDAndID: findRecommendQAWithUIDAndID,
   deleteRecommendQAWithUIDAndID: deleteRecommendQAWithUIDAndID,
   getRecommendQAResponseWithUID: getRecommendQAResponseWithUID,
}